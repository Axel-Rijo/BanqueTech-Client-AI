...
export function MenuPage() {
  ...
  useEffect(() => {
    ...
    const branchExists = branches.some((branch) => branch.id === selectedBranch)
  ...
  useEffect(() => {
    if (activeCategory && tabRefs.current[activeCategory] && tabsListRef.current) {
  ...
  const filterMenuItems = (items) => {
    ...
    return items.filter(
      (item) =>
        ...
        (item.dietary && item.dietary.some((tag) => tag.toLowerCase().includes(lowerCaseSearch))),
    )
  }
  ...
  return (
    ...
    >
          ...
          <CardContent className="p-4">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
              <div className="flex items-center gap-2 mb-4 md:mb-0">
                ...
                <h3 className="font-bold text-lg">{currentBranch?.name || "Selected Branch"}</h3>
                <Button variant="ghost" size="sm" className="ml-2 text-xs" onClick={toggleBranchDetails}>
                  ...
                  >
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Mon-Fri: {currentBranch.hours?.weekdays || "11:00 AM - 10:00 PM"}
                        </p>
                        <p className="text-sm text-muted-foreground">
                          Sat-Sun: {currentBranch.hours?.weekends || "10:00 AM - 11:00 PM"}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">{currentBranch.phone || "(123) 456-7890"}</p>
                      </div>
                    </div>
                  </motion.div>
                </motion.div>
      ...
      <motion.div
        ...
        className="mt-8"
      >
        <Tabs defaultValue={menuCategories[0].id} onValueChange={handleTabChange}>
            ...
            >
              {menuCategories.map((category) => (
                <TabsTrigger
                  key={category.id}
                  value={category.id}
                  className="min-w-fit data-[state=active]:bg-primary data-[state=active]:text-black transition-all duration-200"
                  ref={(el) => {
          ...
          {menuCategories.map((category) => {
            const categoryItems = filterMenuItems(branchMenu.filter((item) => item.category === category.id))
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    {categoryItems.length > 0 ? (
                      categoryItems.map((item, index) => <MenuItem key={item.id} item={item} index={index} />)
                    ) : (
                            ...
                            {searchTerm ? (
                              <>
                                ...
                                <p className="text-muted-foreground">
                                  No items match your search "{searchTerm}" in this category. Try a different search
                                  term or category.
                                </p>
                              </>
                            ) : (
                              <>
                                ...
                                <h3 className="text-xl font-medium mb-2">Not available at this location</h3>
                                <p className="text-muted-foreground">
                                  This menu category is not available at the selected branch. Please select another
                                  category.
                                </p>
                              </>
                            )}
